...
with st.expander("🧪 C - Modélisation (10 à 12 min)", expanded=False):
    ...
    - Modèles testés : linéaire, ElasticNet, KNN, RandomForest, XGBoost,
      SARIMAX, Deep Learning
    ...
    st.markdown(
        """
    - 🥇 **XGBoost** : meilleur compromis généralisation / précision (R²
    jusqu’à 0.95)
    ...
    - 🧠 **Deep Learning (Granite TTM)** :
      ...
      - améliorable par *fine-tuning* (R² ≈ 0.87 en test sur 1 semaine
        glissante)
    """
    )
    ...
    notre laboratoire d'entraînement et d'observation de la performance des
    modèles en pilotant les conditions
    """
    )
...
with st.expander("🔚 D - Conclusion & Ouverture (1 min)", expanded=False):
    st.markdown(
        """
    ✅ Une approche complète mêlant statistique, modélisation ML, séries
    temporelles et deep learning.

    🚀 Prochaine étape : entraînement multi-compteurs sur 8 ans d’archives
    ParisData.

    🔍 Exploiter les séries longues pour **entraîner de meilleurs modèles
    neuronaux**.

    👉 Continuez avec les pages suivantes pour explorer plus interactivement
    nos résultats :
...
st.caption(
    "Projet réalisé dans le cadre de la formation Machine Learning Engineering"
